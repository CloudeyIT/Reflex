                ...
                switch (context.Resource)
                {
                    case IMiddlewareContext middlewareContext:
                    {
                        ...
                        return result switch
                        {
                            ...
                            CollectionSegment<T> pagedResults => pagedResults.Items.All(
                                x => expression.Invoke(x, context, middlewareContext)
                            ),
                            Connection<T> cursorPagedResults => cursorPagedResults.Edges.All(
                                x => expression.Invoke(x.Node, context, middlewareContext)
                            ),
                            Error => true,
                            null => expression.Invoke(default, context, middlewareContext),
                ...
                switch (context.Resource)
                {
                    case IMiddlewareContext middlewareContext:
                    {
                        ...
                        return result switch
                        {
                            ...
                            CollectionSegment<T> pagedResults => (await Task.WhenAll(
                                pagedResults.Items.Select(
                                    x => expression.Invoke(x, context, middlewareContext)
                                )
                            )).All(x => x),
                            Connection<T> cursorPagedResults => (await Task.WhenAll(
                                cursorPagedResults.Edges.Select(
                                    x => expression.Invoke(x.Node, context, middlewareContext)
                                )
                            )).All(x => x),
                            Error => true,
                            null => await expression.Invoke(default, context, middlewareContext),
                            _ => throw new ApplicationException(
                                "Invalid result type for result assertion requirement. Must be the same as the type of the target!"
                            ),
                        };
